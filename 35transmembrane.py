...
for defline, seq in seqs:
	...
	if sig_flag == False:
		for i in range(1, len(nterm) -8 +1):
			...
			lose = nterm[i-1]
			gain = nterm[i+8-1]
			if lose in sequence.AMINOKD:
				kd -= sequence.AMINOKD[lose]
			if gain in sequence.AMINOKD:
				kd += sequence.AMINOKD[gain]
			if kd / 8 >= 2.5 and 'P' not in window:
				sig_flag = True
	...
	if txm_flag == False:
		for i in range(1, len(nterm) -11 +1):
			window = nterm[i:i+11]
			lose = nterm[i-1]
			gain = nterm[i+11-1]
			if lose in sequence.AMINOKD:
	...
	if txm_flag == True: print(defline)